...
use super::interaction::send_or_error;
...
pub(super) fn generation_controls(
    ...
    let generate = generate_handler(state, client.clone(), signals);
    let stop = stop_handler(state, client);
    view! {
        <section class="generation-bar" data-testid="generation-controls" aria-labelledby="prompt-label">
            <div class="generation-primary">
                ...
                <div class="generation-primary-action">
                    <button
                        type="button"
                        class="primary"
                        data-testid="generate"
                        hidden=move || state.with(|current| current.generation.phase == GenerationPhase::Running)
                        aria-busy=move || state.with(|current| current.generation.pending.is_some().to_string())
                        disabled=move || state.with(generate_disabled)
                        on:click=generate
                    >"Generate"</button>
                    <button
                        type="button"
            ...
            {generation_settings(state, signals, set_mode)}
...
fn generation_settings(
    ...
    set_mode: WriteSignal<SamplingMode>,
) -> impl IntoView {
    ...
    view! {
        <details class="generation-settings">
            ...
            <div class="generation-settings-grid">
                {number_field(
                    ...
                    move || state.with(|current| {
                        current.model.as_ref().map_or(1, |model| model.config.block_size).to_string()
                    }),
                    "1",
                )}
            ...
            <div class="generation-applied" role="group" aria-label="Worker 적용 설정">
                {move || state.with(|current| current.generation.config.as_ref().map(|config| view! {
                    <span>{format!("{} tokens", config.max_new_tokens)}</span>
...
const fn generate_disabled(state: &AppState) -> bool {
    matches!(state.status, AppStatus::Loading(_))
        || state.model.is_none()
        || state.generation.pending.is_some()
}
