...
readonly GENERATED_DIR="${ROOT_DIR}/apps/web/src/generated"
readonly WASM_BINDGEN_VERSION="0.2.127"
readonly WASM_OPT_VERSION="123"
mkdir -p "${GENERATED_DIR}" "${OUTPUT_DIR}"
readonly TEMP_DIR="$(mktemp -d "${GENERATED_DIR}/.worker-candidate.XXXXXX")"
...
cd "${ROOT_DIR}"
readonly SOURCE_FINGERPRINT="$(python3 "${ROOT_DIR}/scripts/worker_source_fingerprint.py" "${ROOT_DIR}")"
...
python3 "${ROOT_DIR}/scripts/worker_artifact_install.py" \
  "${ROOT_DIR}" "${TEMP_DIR}" "${OUTPUT_DIR}" "${SOURCE_FINGERPRINT}"
printf 'Generated production Worker artifacts in %s (validated)\n' "${OUTPUT_DIR}"
